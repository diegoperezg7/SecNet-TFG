...
def process_alert(alert_data):
    ...
    try:
        ...
        # Determine action based on severity and alert type
        action_taken = "Logged only"
        
        # Block IP for high severity alerts or specific alert types
        if severity >= 2 or "Port Scan" in alert_message:
            if block_ip(src_ip, alert_message):
        ...
        logger.info(f"Processed alert: {alert_message} from {src_ip} - Action: {action_taken}")
